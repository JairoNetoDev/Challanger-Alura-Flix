...
const gamesApiUrl =
  "https://my-json-server.typicode.com/JairoNetoDev/alura-flix-api/games";
const categoriesApiUrl =
  "https://my-json-server.typicode.com/JairoNetoDev/alura-flix-api/categories";
...
export function useGamesContext() {
  ...
  function deleteGame(game) {
    axios
      ...
      .catch(() => {
        const gameId = games.some((thisGame) => thisGame.id === game.id);
        gameId
          ? setGames(games.filter((thisGame) => thisGame.id !== game.id))
          : alert("Não foi possível deletar o jogo. Tente novamente.");
        console.log(
          "Não foi possível deletar o jogo 100%, mas para fins de teste, entrou no catch."
        );
      });
  }
